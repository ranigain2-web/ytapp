import { InvalidProxyError, SessionManager } from "./session_manager.js";
import { strerror, VERSION } from "./utils.js";
import { Command } from "commander";
import express from "express";
import http from "node:http";
function collectHosts(host, previous) {
    previous.push(...host
        .split(",")
...
const program = new Command()
    .option("-p, --port <PORT>")
    .option("-H, --host <HOST>", "Host/IP to listen on; repeat or separate with commas to bind multiple addresses", collectHosts, [])
    .parse();
...
function getListenAddresses(hosts) {
    if (hosts.length === 0)
        return LISTEN_ADDRESSES;
    return hosts.map((host) => host.includes(":") ? { host, ipv6Only: true } : { host });
}
...
httpServer.post("/get_pot", async (request, response) => {
    ...
    try {
        ...
        if (!(e instanceof InvalidProxyError))
