      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-14 bg-[#0f0f0f] flex items-center gap-2 px-2 sm:px-4">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            aria-label="Guide"
            className="w-11 h-11 rounded-full flex items-center justify-center hover:bg-[#272727] active:bg-[#3f3f3f] transition-colors"
          >
        ...
        {route.name === "search" && route.q ? (
          <button
            ...
            className="sm:hidden flex flex-1 h-10 mx-1 rounded-full bg-[#121212] border border-[#303030] items-center gap-3 px-4 min-w-0"
            data-testid="mobile-query-bar"
            aria-label={`Search: ${route.q}`}
          >
            <Search className="w-5 h-5 text-[#aaa] shrink-0" />
            <span className="text-[15px] text-[#f1f1f1] truncate">{route.q}</span>
          </button>
        ) : (
        ...
        <div className="flex items-center gap-1 shrink-0 sm:hidden">
          <button
            ...
            className="w-11 h-11 rounded-full flex items-center justify-center hover:bg-[#272727] active:bg-[#3f3f3f]"
          >
            <Search className="w-6 h-6 text-[#f1f1f1]" />
          </button>
          <button
          ...
          <div className={`flex w-full max-w-[540px] min-w-0 ${focus ? "md:translate-x-[68px]" : ""} transition-transform duration-200`}>
            <div className="relative flex flex-1 min-w-0">
              ...
              {suggOpen && focus && desktopList.length > 0 && (
                <div
                  ref={suggBoxRef}
                  className="absolute top-11 left-0 right-0 rounded-xl bg-[#212121] border border-[#303030] shadow-2xl py-2 z-50"
                  ...
                  {desktopList.map((item, i) => (
                    ...
                    >
                      {("recent" in item && item.recent)
                        ...
                        : <Search className="w-5 h-5 text-[#aaa] shrink-0" />}
                      <span className="text-[15px] text-[#f1f1f1] truncate pr-2">{item.text}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="hidden sm:flex items-center gap-1 shrink-0">
          <button className="hidden md:flex w-10 h-10 rounded-full items-center justify-center hover:bg-[#272727]" aria-label="Cast">
            <Cast className="w-6 h-6 text-[#f1f1f1]" />
          </button>
          <button className="hidden md:flex w-10 h-10 rounded-full items-center justify-center hover:bg-[#272727] relative" aria-label="Notifications">
